...
client = FaunaClient(secret='fnAFlLDycCAAQpajHbgEmDY86Nvw8AWuWud3LizY',domain="db.us.fauna.com")
import requests
import json
...
root_node = '9TqvnNUaKUF162BatKE6A8BY3ydokCptWCQcH9NFkA9v'#New
...
def home():
    ...
    while True:
        ...
        try:
            ...
            # result = client.query(q.map_(q.lambda_("x", q.get(q.var("x"))), q.paginate(q.documents(q.collection("lp_v2"))))) ##lp_v2
            # data = [doc["data"] for doc in result["data"]]
            # tokenA = [doc["data"]['baseMint'] for doc in result["data"]]
            # # print(tokenA[-1:])
            # mintA = [doc["data"]['quoteMint'] for doc in result["data"]]
            # precision_list = [doc['precision'] for doc in data if 'precision' in doc]
            # if precision_list:
            #     for precision in precision_list:
            #         precision = int(precision)  # Convert each precision to int and print
            #         print(f'precision {precision}')

            # frozen = [doc["data"]['isFrozen'] for doc in result["data"]]
            # print(frozen)
            # frozen = str(frozen)
            import json
            ...
            with open("deployed.txt", "w", encoding="utf-8") as txt_file:
                    txt_file.write(mintA)  # Write the entire tokenA string on a single line
            ...
            for token in mintA:
                    ...
                    url = f"https://api.dexscreener.com/latest/dex/tokens/{mintA}"
                    resp = httpx.get(url)
                    ...
                    #     print('skipping')
                    # else:
                    #     post = client.query(q.create(
                    #         q.collection('hopebot_datafeeds'),
                    #         {'data': result}
                    #     ))
                    # time.sleep(1)
                    ...
                    # data_training = [doc["data"] for doc in result["data"]]
                    ...
                    #     # Delete all rows in hopebot_datafeeds collection
                    #     # for doc in result_training["data"]:
                    #     #     client.query(q.delete(doc['ref']))
                    #     # print('All hopebot_datafeeds rows deleted')
                    ...
                    with open(csv_file_path, mode='a', newline='') as csv_file:
                        ...
                        for item in data_training:
                            ...
                            # pair_address = item['result'][0]['pairAddress']
                            # base_address = item['result'][0]['baseToken']['address']
                            # quote_address = item['result'][0]['quoteToken']['address']
                            # h24_volume = item['result'][0]['volume']['h24']
                            # h1_volume = item['result'][0]['volume']['h1']
                            # m5_volume = item['result'][0]['volume']['m5']
                            # m5_buy = item['result'][0]['txns']['m5']['buys']
                            # m5_sell = item['result'][0]['txns']['m5']['sells']
                            # m5_change = item['result'][0]['priceChange']['m5']
                            # price = item['result'][0]['priceUsd']
                            # liquidity = item['result'][0]['liquidity']['usd']
                            # liquiditySol = item['result'][0]['liquidity']['quote']
                            # dex = item['result'][0]['dexId']
                            # result = item.get('result', [])[0]  # Get the first element in 'result' list
                            ...
                            dex = item.get('dexId', '')
                            balance = token_balance(root_node, mintA)
                            amountIn = 10
                            ...
                            if dex == 'raydium' or 'pumpfun':
                                ...
                                print(f'\n\n\n\n\n OPEN DEXSCREENER TOKEN PORTFOLIO/CHART: \n \n\n\n\n https://dexscreener.com/solana/{pair_address}?maker={root_node} \n\n\n')
            ...
            # price =
                                    ...
                                    priceChange = a['Price Change (5m)'][-1:]
                                    ...
                                    pred1_int = int(pred1)
                                    pred_int = int(pred)
                                    root_node = '9TqvnNUaKUF162BatKE6A8BY3ydokCptWCQcH9NFkA9v'
                                    ...
                                    price_score = float(score1) if not math.isnan(float(score1)) else 50.0
                                    rsi_score = float(score2) if not math.isnan(float(score2)) else 50.0
                                    ...
                                    if (float(a['Balance'][-1:])<1 or float(pred1) <= effective_price or float(priceChange) <0.04 or float(priceChange) >0.05 or  0.03 <= float(priceChange) <= 0.06 or (float(first_iteration['Liquidity (USD)'][-1:]) <= float(liquidity) and solBalance >= 0.01) or (solBalance >= 0.01) or rsi_last<=65.00  or float(price)<float(a['Price (Native)'][-1:])) and float(pred) <= float(liquidity) or amountOut<=0:
                                    ...
                                    # if float(pred1) <= float(price) or float(first_iteration['Liquidity (USD)'][-1:]) >=3.00 and solBalance>=0.029 or rssi>=20.00: # or float(pred2)>=42.0 and float(pred2)<=93
                                        ...
                                        # a.to_csv('side.csv')
                                        holders = get_token_accounts('UQ5q7mjDRynZnV5RX5YfbaJMnXdmRaSX61c2J2wBsoi')
                                        price_score = float(score1) if not math.isnan(float(score1)) else 0.0
                                        rsi_score = float(score2) if not math.isnan(float(score2)) else 0.0
                                    ...
                                    ########SELLLLLLLL
                                    if (solBalance <0.02  or rsi_last>=58.00 or float(priceChange) >=100.00 or float(priceChange) >=.97 or pred_price_change >= 0.80 or float(effective_price) >= float(pred1)  or float(price) >=float(pred1)  or float(price2[-1:])>=target_sell_price or float(priceChange)>=100.00) and balance >= amountOut or balance >= amountOut and float(liquidity) >= float(first_iteration['Liquidity (USD)'][:-1]) or  balance >= amountOut and float(first_iteration['Liquidity (USD)'][-1:]) <= float(first_iteration['Liquidity (USD)'][-2:-1])*.3: #or float(pred2) >=53.0
                                        ...
                                        root_node = '9TqvnNUaKUF162BatKE6A8BY3ydokCptWCQcH9NFkA9v'
                                        post = {'baseMint':base_address,'quoteMint':quote_address,'pair_address':pair_address,'price_score':float(score1),'rsi_score':float(score2),'liquidity_prediction':float(pred),'rating':'1','side':'Sell','price':float(price),'price_prediction':float(pred1),'amount':amountOut,'rsi_prediction':float(pred2),'Balance':balance,'effective_price':effective_price,'transferAmount':transferAmount}# 'predictedSellPrice':pred_sell,'predictedBuyPrice':pred_buy,'buyPrice':solana_buy,'sellPrice':solana_sell,'solanaPrice':solana_price,'predictedSolanaPrice':pred_open
                                        # client.query(q.create(
                                        #         q.collection('hopebot_predictions'),
                                        #         {'data':post}
                                        #     ))
